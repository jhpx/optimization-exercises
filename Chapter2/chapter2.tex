...
\begin{solution}
    \begin{tikzpicture}
        \begin{axis}[
            ...
            \addplot[name path=a,color=red,domain=0:15] {x};
            \addlegendentry{$7x_1 + 3x_2=19$}
            \addplot[name path=b,color=blue,domain=0:15] {(66-6*x)/11};
            \addlegendentry{$10x_1 + 2x_2=11$}
            \addplot[name path=d,color=black,dashed] {(44.5-5*x)/13};
            \addplot[geyecolor] fill between [of=a and b, split,every last segment/.style=gray!50];
        \end{axis}
    \end{tikzpicture}
